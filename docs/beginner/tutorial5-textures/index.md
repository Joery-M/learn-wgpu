...
Remember the `PipelineLayout` we created back in [the pipeline section](../tutorial3-pipeline/index.md#how-do-we-use-the-shaders)? Now, we finally get to use it! The `PipelineLayout` contains a list of `BindGroupLayout`s that the pipeline can use. Modify `render_pipeline_layout` to use our `texture_bind_group_layout`.
